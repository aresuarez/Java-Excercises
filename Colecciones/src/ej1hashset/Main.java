...
public class Main {
	...
	public static void main(String[] args) {
		...
		HashSet <Anotador> list = new HashSet<Anotador>();
		int n = 0;
		do {
			...
			case 3:
				...
				for(Anotador i: list) {
					if(i.getCiudad().equalsIgnoreCase(provin)) {
						System.out.println(i.toString());
					}}
				break;
			case 4:
				...
				for(Anotador i: list) {
					if(i.getNombre().equalsIgnoreCase(name)) {
						i.setNumCanastasIntroducidas(numCanas);
					}}
				break;
			case 5:
				...
				for(Anotador i: list) {
					if(i.getNumCanastasIntroducidas() < rangTop && i.getNumCanastasIntroducidas() > rangLow) {
						list.remove(i);
					}}
